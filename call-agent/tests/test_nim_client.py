"""
Tests for the NVIDIA NIM intelligence client in Development Test Mode.
...
def test_sanitize_speech_output():
    ...
    # Test 4: Assistant prefix
    prefixed_text = 'Assistant: "Tomorrow morning is safe for fishing."'
    assert sanitize_speech_output(prefixed_text) == "Tomorrow morning is safe for fishing."


@pytest.mark.asyncio
async def test_nim_client_success_telugu():
    """Verify NIM API successfully returns Telugu spoken response."""
    client = NIMClient(api_key="nvapi-test", model="openai/gpt-oss-20b", timeout=5.0)
...
@pytest.mark.asyncio
async def test_nim_client_hindi_response():
    """Verify NIM API returns natural Hindi spoken response."""
    client = NIMClient(api_key="nvapi-test", model="openai/gpt-oss-20b", timeout=5.0)
    url = "https://integrate.api.nvidia.com/v1/chat/completions"
...
@pytest.mark.asyncio
async def test_nim_client_english_response():
    """Verify NIM API returns English spoken response."""
    client = NIMClient(api_key="nvapi-test", model="openai/gpt-oss-20b", timeout=5.0)
    url = "https://integrate.api.nvidia.com/v1/chat/completions"
...
@pytest.mark.asyncio
async def test_nim_client_timeout_and_fallback():
    """Verify graceful fallback response when NIM API request times out."""
    client = NIMClient(api_key="nvapi-test", model="openai/gpt-oss-20b", timeout=0.1, max_retries=1)
    url = "https://integrate.api.nvidia.com/v1/chat/completions"
...
@pytest.mark.asyncio
async def test_nim_client_missing_key_fallback(monkeypatch):
    """Verify fallback response when NVIDIA_API_KEY is empty."""
    monkeypatch.setattr(settings, "NVIDIA_API_KEY", "")
    client = NIMClient(api_key="", model="openai/gpt-oss-20b")
    res = await client.query(
...
@pytest.mark.asyncio
async def test_nim_client_http_429_quota_limit():
...
@pytest.mark.asyncio
async def test_nim_client_client_error_400_and_401():
    """Verify HTTP 400 and 401 client errors return voice fallback with 0 retries."""
    client = NIMClient(api_key="invalid-key", model="openai/gpt-oss-20b", timeout=5.0)
    url = "https://integrate.api.nvidia.com/v1/chat/completions"
...
@pytest.mark.asyncio
async def test_nim_client_empty_or_malformed_response():
...
@pytest.mark.asyncio
async def test_backend_client_routes_to_nim_when_test_mode_enabled(monkeypatch):
    """Verify AIBackendClient delegates to the NIM client when CALL_AGENT_TEST_MODE is True."""
    ...
    monkeypatch.setattr(
        nim_client,
        "query",
        AsyncMock(return_value=AIQueryResponse(response="NIM test mode response", language="en-IN", priority="normal")),
    )
    ...
    assert result.response == "NIM test mode response"
    assert result.language == "en-IN"
    ...
    with respx.mock(base_url="http://mock-backend:8080") as respx_mock:
        respx_mock.post("/api/ai/query").mock(
            return_value=httpx.Response(
                200,
                json={
                    "response": "Production marine agent response",
                    "language": "en-IN",
                    "priority": "normal",
                },
            )
        ...
        assert result.response == "Production marine agent response"
        assert result.language == "en-IN"

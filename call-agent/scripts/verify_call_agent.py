...
"""Check every credential and every hop the phone agent depends on.
    ...
    python scripts/verify_call_agent.py

Run it as `python scripts/verify_call_agent.py` from call-agent/, with the data API running.
...
broken rather than leaving you to infer it from a caller hearing silence:
    ...
    fisherman -> Exotel -> ngrok -> call agent -> data API -> INCOIS/NVIDIA NIM
                                       |
...
# scripts/ -> call-agent/, and the data API lives in ../backend.
...
def main() -> int:
    ...
    for name, value in required.items():
        ...
        if not value:
            report("FAIL", f"{name} is set", "not present in call-agent/.env")
    ...
    backend_key = api_env.get("NVIDIA_API_KEY", "") or api_env.get("NVIDIA_API_KEYS", "").split(",")[0]
    if backend_key.startswith("nvapi-") and len(backend_key) > 40:
        report("PASS", "data API NVIDIA_API_KEY looks real", masked(backend_key))
    else:
        report("FAIL", "data API NVIDIA_API_KEY looks real",
               f"the data API .env has {masked(backend_key)} — the agent cannot reason without it")

    report("PASS", "data API NIM model", api_env.get("NIM_MODEL", "openai/gpt-oss-20b (default)"))
    ...
    try:
        ...
        report("FAIL", "data API is running",
               f"{api_url} unreachable: {exc}\nStart it with:  python backend/api_server.py")

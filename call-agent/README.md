...
│                                                             │
│  5. AI Intelligence Engine (`app/ai/backend_client.py`)     │
│     - Data API marine agent on NVIDIA NIM (tool calling)    │
...
│   ├── ai/
│   │   ├── __init__.py
│   │   ├── backend_client.py    # Marine agent connector (data API, NIM tool calling)
│   │   ├── nim_client.py        # Direct NVIDIA NIM client (CALL_AGENT_TEST_MODE only)
│   │   └── voice_text.py        # Persona, spoken fallbacks, language detection
│   ├── conversation/
│   │   ├── __init__.py
...
| `AI_PROVIDER` | `backend` | Always the data API's marine agent (NVIDIA NIM, tool calling) |
| `CALL_AGENT_TEST_MODE` | `false` | Answer from NIM directly, without marine tools, to test audio only |
| `NVIDIA_API_KEY` | - | NVIDIA NIM key, used directly only in test mode |
| `NIM_MODEL` | `openai/gpt-oss-20b` | NIM model with tool calling support |
| `NIM_BASE_URL` | `https://integrate.api.nvidia.com/v1` | NIM API base URL |
| `NIM_MAX_TOKENS` | `600` | Max tokens per spoken turn (includes reasoning) |
| `LLM_TIMEOUT_SECONDS` | `20.0` | Timeout for NIM requests |
| `LLM_MAX_RETRIES` | `1` | Retry attempts for NIM requests |

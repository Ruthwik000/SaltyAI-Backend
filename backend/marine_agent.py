"""NVIDIA NIM tool-calling agent for the SALTY data layer.
...
class _RateLimited(Exception):
    """The LLM API said 429, usually with how long to wait."""
...
def _retry_after_seconds(detail: str, headers: Any = None, default: float = 5.0) -> float:
    """How long the API wants us to wait, from its message or its headers."""
...
class AgentError(RuntimeError):
    """Base exception for LLM API and tool-calling errors."""
    pass


class ERDDAPTools:
    """Allowlist of data tools exposed to the model."""
    ...
    def get_region_data(self, dataset_id: str, variable: str, bbox: list[float], time_range: list[str]) -> dict[str, Any]:
        if len(time_range) != 2:
            raise ValueError(f"time_range needs exactly a start and an end, got {len(time_range)} entries")
        return self._normalized(self.client.get_region_data(dataset_id, variable, bbox, (time_range[0], time_range[1])))
    ...
    def resolve_place(self, name: str) -> dict[str, Any]:
        """A harbour or coastal town name to a position and state."""
        found = resolve_port(name)
        if not found:
            return {"status": "NOT AVAILABLE",
                    "reason": f"'{name}' is not a coastal place SALTY knows; ask for a nearby harbour."}
        return {"source": "SALTY coastal places table", **found}

    def assess_trip_risk(self, destination_latitude: float, destination_longitude: float,
                         departure_latitude: float | None = None,
                         departure_longitude: float | None = None,
                         destination_name: str | None = None) -> dict[str, Any]:
        """The SALTY risk score for a trip, from INCOIS conditions along the track."""
        lat = departure_latitude if departure_latitude is not None else self.latitude
        lon = departure_longitude if departure_longitude is not None else self.longitude
        if lat is None or lon is None:
            return {"status": "NOT AVAILABLE", "reason": "no departure position available"}
        try:
            return field_api.trip_risk({
                "departureLat": lat, "departureLon": lon, "departurePort": self.place,
                "destinationLat": destination_latitude, "destinationLon": destination_longitude,
                "destinationZoneName": destination_name,
            })
        except (RouteError, ThreddsError, KeyError, TypeError, ValueError) as exc:
            return {"status": "NOT AVAILABLE", "reason": str(exc)[:200]}

    def predict_search_area(self, last_known_latitude: float, last_known_longitude: float,
                            elapsed_hours: float, target_type: str = "craft") -> dict[str, Any]:
        """Search-and-rescue drift datum and search radius from live INCOIS wind and current."""
        try:
            return field_api.sar_predict({
                "lastKnownLat": last_known_latitude, "lastKnownLon": last_known_longitude,
                "elapsedHours": elapsed_hours, "targetType": target_type,
            })
        except (ThreddsError, KeyError, TypeError, ValueError) as exc:
            return {"status": "NOT AVAILABLE", "reason": str(exc)[:200]}

    def get_fleet_status(self, latitude: float | None = None,
                         longitude: float | None = None) -> dict[str, Any]:
        """Boats whose trips were registered from the SALTY app and are still at sea."""
        lat = latitude if latitude is not None else self.latitude
        lon = longitude if longitude is not None else self.longitude
        units = field_api.fleet(float(lat or 0.0), float(lon or 0.0))
        return {
            "source": "SALTY trip registrations (app pings)",
            "note": "Only boats that started a trip in the SALTY app. There is no AIS feed.",
            "count": len(units),
            "overdue": sum(1 for u in units if u["status"] == "overdue"),
            "units": units[:20],
        }
    ...
    def execute(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        functions: dict[str, Callable[..., dict[str, Any]]] = {
            ...
            "resolve_place": self.resolve_place,
            "assess_trip_risk": self.assess_trip_risk,
            "predict_search_area": self.predict_search_area,
            "get_fleet_status": self.get_fleet_status,
        }
...
COMPACT_DESCRIPTIONS = {
    ...
    "resolve_place":
        "Latitude, longitude and state for a harbour or coastal town the user names.",
    "assess_trip_risk":
        "Risk score and level for a trip to a destination latitude/longitude, from live conditions along the track.",
    "predict_search_area":
        "Search-and-rescue drift: datum, search radius and pattern for a missing boat or person, from last known position and hours elapsed.",
    "get_fleet_status":
        "Boats at sea that registered a trip in the SALTY app, including overdue ones.",
    "get_hazard_zones":
...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "function": {
            "name": "resolve_place",
            "description": "Convert a harbour, fishing village or coastal town name into latitude, longitude and state. Call it before any position-based tool when the user names a place other than their selected location.",
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "assess_trip_risk",
            "description": "SALTY risk score (0-100) and level for a trip from the user's position (or a given departure) to a destination, scored leg by leg on live INCOIS conditions, with thunderstorm outlook and EEZ distance. Use for trip planning questions that name a destination.",
            "parameters": {
                "type": "object",
                "properties": {
                    "destination_latitude": {"type": "number"},
                    "destination_longitude": {"type": "number"},
                    "departure_latitude": {"type": "number"},
                    "departure_longitude": {"type": "number"},
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "predict_search_area",
            "description": "Search-and-rescue drift projection for a missing vessel or person in water: leeway from live INCOIS wind, search radius widened by live current, datum position, drift path and recommended search pattern. Use for lost fisherman, missing boat, drift or SAR questions.",
            "parameters": {
                "type": "object",
                "properties": {
                    "last_known_latitude": {"type": "number"},
                    "last_known_longitude": {"type": "number"},
                    "elapsed_hours": {"type": "number"},
                    "target_type": {"type": "string", "enum": ["piw", "craft", "trawler", "raft"],
                                    "description": "piw = person in water."},
                },
                "required": ["last_known_latitude", "last_known_longitude", "elapsed_hours"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "description": "Boats currently at sea that registered a trip from the SALTY app, nearest first, with last ping, heading, destination and overdue status. Use for coastal operator questions about the fleet, who is at sea, or overdue boats. There is no AIS feed.",
            "parameters": {
                "type": "object",
...
# NVIDIA NIM hosted inference (OpenAI-compatible). gpt-oss-20b is the default
# because it supports native tool calling on NIM and the prompts in this file
# were tuned on it; any NIM model with tool calling can be set via NIM_MODEL.
DEFAULT_NIM_BASE_URL = "https://integrate.api.nvidia.com/v1"
DEFAULT_NIM_MODEL = "openai/gpt-oss-20b"


class MarineAgent:
    """SALTY marine agent: NVIDIA NIM chat completions with OpenAI-compatible tool calling."""
    ...
    def __init__(
        ...
        self.tools = tools
        self.model = model or os.getenv("NIM_MODEL", DEFAULT_NIM_MODEL)
        self.base_url = (base_url or os.getenv("NIM_BASE_URL", DEFAULT_NIM_BASE_URL)).rstrip("/")
        # One key, or several. NIM rate limits are per account, so keys from
        # different accounts are separate budgets, and rotating to the next one
        # on a 429 is instant where waiting is silence on a live call.
        # NVIDIA_API_KEYS takes a comma-separated list; NVIDIA_API_KEY still
        # works on its own.
        listed = [k.strip() for k in os.getenv("NVIDIA_API_KEYS", "").split(",") if k.strip()]
        single = api_key or os.getenv("NVIDIA_API_KEY", "")
    ...
    def _chat(self, messages: list[dict[str, Any]], include_tools: bool = True,
        ...
        """One chat completion, retrying when NIM asks us to wait."""
        if not self.api_keys:
            raise AgentError("NVIDIA_API_KEY environment variable is not configured")

        payload_data: dict[str, Any] = {
            "model": self.model,
            "messages": messages,
            "temperature": 0.2,
            # An uncapped model will spend hundreds of tokens on an answer that
        ...
        # Rotations and waits are counted separately, so trying a second key
    ...
    # gpt-oss counts its reasoning against max_tokens, hence the headroom.
    _MAX_COMPLETION = {"voice": 600, "normal": 900, "research": 2500}
    ...
    def _send(self, payload: bytes, headers: dict[str, str]) -> dict[str, Any]:
        ...
        try:
            ...
            # NIM explains every rejection in the response body. urllib's str()
            # shows only "HTTP Error 403: Forbidden", so pass the real reason on.
            try:
            ...
            raise AgentError(
                f"NIM API returned HTTP {exc.code} for model {self.model}: "
                f"{detail or exc.reason}"
            ) from exc
        except (OSError, URLError, json.JSONDecodeError) as exc:
            raise AgentError(f"NIM API is unavailable at {self.base_url}: {exc}") from exc

        choices = data.get("choices") or []
        if not choices:
            return {"message": {}}
        raw = choices[0].get("message") or {}
        # Keep only what goes back into the conversation. NIM adds fields such
        # as reasoning_content that some models reject when echoed back.
    ...
    _SOURCE_PAGES: dict[str, tuple[str, str]] = {
        ...
        "assess_trip_risk": (
            "INCOIS Ocean State Forecast, scored with the SALTY risk model",
            "https://www.incois.gov.in/oceanservices/osfforecast.jsp"),
        "predict_search_area": (
            "INCOIS Ocean State Forecast wind and current",
            "https://www.incois.gov.in/oceanservices/osfforecast.jsp"),
        "search_datasets": (
    ...
    def _references(cls, calls: list[dict[str, Any]],
        ...
        for call in calls:
            name = call.get("tool")
            if not isinstance(name, str) or name not in usable:
                continue
    ...
    def answer(
        ...
        coverage = (
            ...
            " resolve_place turns a harbour or town the user names into a position;"
            " call it before position tools when they name somewhere other than their"
            " selected location. assess_trip_risk gives the SALTY risk score and level"
            " for a trip to a destination. predict_search_area gives a search-and-rescue"
            " drift datum, radius and pattern from the last known position and hours"
            " elapsed - use it for any lost boat, person in water or drift question."
            " get_fleet_status lists boats that registered a trip in the SALTY app and"
            " which are overdue; there is no AIS feed, so say so if asked about other"
            " vessels.\n"

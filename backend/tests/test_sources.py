...
Run this before trusting an answer, and after any change to a client module:
    ...
    python -m tests.test_sources                 # every source, from backend/
    python -m tests.test_sources --only tides    # one source
...
def check_llm():
    ...
    if not (os.environ.get("NVIDIA_API_KEY") or os.environ.get("NVIDIA_API_KEYS")):
        raise AssertionError("NVIDIA_API_KEY is not set in the environment")
    from marine_agent import TOOL_DEFINITIONS

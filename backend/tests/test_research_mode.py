...
The LLM is mocked here on purpose. These are tests of the LOOP - how many rounds
...
def agent(replies: list[dict], **kwargs) -> tuple[MarineAgent, list]:
    """An agent whose model returns `replies` in order. Records every request."""
    seen: list = []
    instance = MarineAgent(
...
def stub_tools(instance: MarineAgent, payload: dict) -> None:

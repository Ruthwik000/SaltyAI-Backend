"""
Production Groq Intelligence Client for SALTY AI Call Agent.
...
class GroqClient:
    """Production client for Groq API conversational voice reasoning."""
    ...
    async def query(
        ...
        if not api_key:
            logger.warning(
                f"[GROQ CLIENT] GROQ_API_KEY is not configured. Returning voice fallback response for call {call_id}."
            )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if generated_text:
                        ...
                        logger.info(
                            f"[GROQ CLIENT] Success in {latency_ms:.1f}ms | Call {call_id} | "
                        ...
                        logger.warning(f"[GROQ CLIENT] Received empty choices from Groq API for call {call_id}")
                        break

                elif response.status_code == 429:
                    logger.warning(
                        f"[GROQ CLIENT] HTTP 429 Rate Limit Exceeded for call {call_id}. "
                        f"Returning immediate voice fallback."
                    )
                    break
                elif response.status_code in (400, 401, 403, 404):
                    logger.error(
                        f"[GROQ CLIENT] Client error HTTP {response.status_code}: {response.text} | call_id: {call_id}"
                    )
                    break
                elif response.status_code in (500, 502, 503, 504) and attempt < max_retries:
                    logger.warning(
                        f"[GROQ CLIENT] HTTP {response.status_code} (attempt {attempt + 1}/{max_retries + 1}). "
                        f"Retrying..."
                    )
                    ...
                    logger.error(
                        f"[GROQ CLIENT] API error HTTP {response.status_code}: {response.text} | call_id: {call_id}"
                    )
                    break

            except httpx.TimeoutException:
                if attempt < max_retries:
                    logger.warning(f"[GROQ CLIENT] Request timeout (attempt {attempt + 1}). Retrying...")
                    await asyncio.sleep(0.3)
                    continue
                logger.error(f"[GROQ CLIENT] Request timed out after {timeout}s for call {call_id}")
                break

            except Exception as e:
                logger.error(f"[GROQ CLIENT] Unexpected error calling Groq API: {e} | call_id: {call_id}", exc_info=True)
                break
